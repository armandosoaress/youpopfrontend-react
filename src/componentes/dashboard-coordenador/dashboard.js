...
function Dashboardlayout() {
    ...
    const [recrutador, setrecrutadores] = useState([]);
    ...
    var supervisoresoptions, recrutadoresoptions, coordenadoresoptions, chooptions, optionsocoordenadorporid = "";
    ...
    const addoptions = async (responseJson) => {
        ...
        if (localStorage.getItem('user_permition') == 2) {
            fetch("http://localhost/youpop-plano-eusouinevitavel/backend/api/chousers?id=" + localStorage.getItem('id_user_log'), {
                method: 'get',
                headers: header,
            })
                .then((response) => response.json())
                .then((responseJson) => (
                    Object.values(responseJson.Coordenadores).map(coordenadores => (
                        optionsocoordenadorporid += "<option value='" + coordenadores.id + "'>" + coordenadores.name + "</option>"

                    )),
                    critaruser(supervisoresoptions, recrutadoresoptions, optionsocoordenadorporid, chooptions)
                ));
                return
        }
    ...
    const insertuser = async (responseJson) => {
        setData(responseJson.Coordenadores);
    }
    const recrutadoresuser = async (responseJson) => {
        if (responseJson.message == "página não disponível") {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Nenhum recrutador encontrado!',
            })
            return
        }
        setrecrutadores(responseJson.Recrutadores);
    }
    ...
    function critaruser(supervisoresoptions, recrutadoresoptions, coordenadoresoptions, chooptions) {
        const { value: formValues } = Swal.fire({
            ...
            html:
                ...
                '<p></p>' +
                '<select id="access" style="margin-top:16px; float:left;margin-left:93px;border:1px solid #dcd7d7;padding:14px;cursor:pointer;" > <option value="Coordenador">Coordenador</option><option value="Recrutador">Recrutador</option><option value="Supervisor">Supervisor</option><option value="motoboy">Motoboy</option></select>',
        }).then((result) => {
            ...
            if (result.isConfirmed) {
                switch (document.getElementById("access").value) {
                    ...
                    case "Coordenador":
                        ...
                        if (localStorage.getItem('user_permition') == 2) {
                            create(localStorage.getItem('id_user_log'), "Coordenador")
                        }
    ...
    // essa funçao serve pra pgar o primeiro id do usuario que fica na hierarquia nessa tela por 
    //exemplo lista os corrdenadores de os recrutafoes entao ele pega o primeiro coordeandor e traz seus recrutadores
                    ...
                    <div className="flex-grow flex overflow-x-hidden">
                        <div className="xl:w-72 w-48 flex-shrink-0 border-r border-gray-200 dark:border-gray-800 h-full overflow-y-auto lg:block hidden p-5">
                            ...
                            <div className="space-y-4 mt-3">
                                {
                                    Object.values(data).map(coordenadores => (
                                        hierarchy(coordenadores.id),
                                        <>
                            ...
                            <div className="sm:p-7 p-4">
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="text-gray-600 dark:text-gray-100">
                                        ...
                                        {
                                            Object.values(recrutador).map(recrutador => (

                                                <>
                                                    <tr className='usersdados' >
                                                        <td onClick={() => redirectdependent(recrutador.id)} className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800">
                                                        ...
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800 md:table-cell hidden"> {recrutador.telefone}</td>
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800 text-red-500"> {recrutador.endereco}</td>
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800">
                                                            <div className="flex items-center">
                                                                ...
                                                                <button onClick={() => editar(recrutador.id)} className="w-8 h-8 inline-flex items-center justify-center text-gray-400 ml-auto">
