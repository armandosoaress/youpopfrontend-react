...
function Dashboardsupervisorlayout() {
    ...
    const getUsers = async () => {
        ...
        fetch("https://armandosoares.com.br/tanayoupop/backend/api/recrutadorusers?id="+ localStorage.getItem('user_recrutador'), {
            method: 'get',
            headers: header,
        })
            .then((response) => response.json())
            .then((responseJson) => (
                supervisoruser(responseJson)
            ));
        ...
        fetch("https://armandosoares.com.br/tanayoupop/backend/api/supervisorusers?id="+ localStorage.getItem('user_supervisor'), {
            method: 'get',
            headers: header,
        })
            .then((response) => response.json())
    ...
    const supervisoruser = async (responseJson) => {
        setsupervisorusers(responseJson.Supervisores);
    }
                ...
                <div className="flex-grow overflow-hidden h-full flex flex-col">
                    ...
                    <div className="flex-grow flex overflow-x-hidden">
                        <div className="xl:w-72 w-48 flex-shrink-0 border-r border-gray-200 dark:border-gray-800 h-full overflow-y-auto lg:block hidden p-5">
                            ...
                            {
                                            Object.values(supervisorusers).map(Supervisores => (

                                                <>
                                                     <button onClick={() => redirect(Supervisores.id)} className="bg-white p-3 w-full flex flex-col rounded-md dark:bg-gray-800 shadow">
                                                ...
                                                <div className="flex items-center w-full">
                                                    <div className="text-xs py-1 px-2 leading-none dark:bg-gray-900 bg-yellow-100 text-yellow-600 rounded-md">  {Supervisores.funcao}</div>
                                                </div>
                                            </button>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="text-gray-400">
                                            ...
                                            <th className="font-normal px-3 pt-0 pb-3 border-b border-gray-200 dark:border-gray-800">Moto</th>
                                    ...
                                    <tbody className="text-gray-600 dark:text-gray-100">
                                       
                                        {
                                            Object.values(data).map(motoboys => (
                                                ...
                                                <>
                                                    <tr className='usersdados' onClick={() => redirectdependent()}>
                                                        ...
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800 md:table-cell hidden"> {motoboys.telefone}</td>
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800 text-red-500"> {motoboys.endereco}</td>
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800 text-red-500"> {motoboys.modelo_da_moto}</td>
                                                        <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800">
                                                            <div className="flex items-center">
...
export default Dashboardsupervisorlayout;
